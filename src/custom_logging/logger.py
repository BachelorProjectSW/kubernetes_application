...
def generate_summary(config_id: str | None = None) -> dict:
    """Generate summary statistics from stored request logs."""
    effective_config_id = _current_config_id() if config_id is None else config_id
    request_logs = get_logs(RequestLog, effective_config_id)

    if not request_logs:
        return {"error": "No request logs available"}

    total_requests = len(request_logs)
    avg_latency_ms = round(sum(entry.latency_ms for entry in request_logs) / total_requests, 2)
    cluster_distribution: dict[str, int] = {}
    total_gco2_g = 0.0
    total_cost_eur = 0.0
    avg_renewable_pct = round(
        sum(entry.renewable_fraction for entry in request_logs) * 100 / total_requests,
        2,
    )
    ...
    for entry in request_logs:
        cluster_distribution[entry.cluster] = cluster_distribution.get(entry.cluster, 0) + 1

        energy_kwh = (entry.cluster_load_w * entry.latency_ms) / 3_600_000_000
        total_gco2_g += entry.blended_carbon_gco2_per_kwh * energy_kwh
        total_cost_eur += entry.blended_cost_eur_per_kwh * energy_kwh
        ...
        cost_over_time.append(
            {
                "timestamp": entry.timestamp,
                "cost_eur": round(entry.blended_cost_eur_per_kwh * energy_kwh, 6),
                "cluster": entry.cluster,
            }
        )
...
def save_summary(summary: dict, config_id: str | None = None) -> None:
    """Persist a generated summary as terminal debug output for later inspection."""
    log.info(
        "custom_logging.summary.generated",
        config_id=_current_config_id() if config_id is None else config_id,
        summary=summary,
    )
