...
from ...custom_logging.log_queue import log_request_fresh_async
...
async def handle_llm_request(question: QuestionConfig, trace_id: str | None = None):
    ...
    if test_state.is_stopping():
        raise HTTPException(
            status_code=503,
            detail="Test stopped during runtime data fetch",
        )
    ...
    if test_state.is_stopping():
        raise HTTPException(
            status_code=503,
            detail="Test stopped before forwarding to cluster",
        )
    try:
