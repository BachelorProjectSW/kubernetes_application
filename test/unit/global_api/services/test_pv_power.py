...
from global_api.services.pv_power import get_pt_power_factor_by_time
...
def test_get_pt_power_returns_only_rows_within_range():
    ...
    with patch("pathlib.Path.open", mock_open(read_data=CSV_CONTENT)):
        result = get_pt_power_factor_by_time(datetime(2010, 6, 1, 11), datetime(2010, 6, 1, 12))
...
def test_get_pt_power_returns_empty_list_if_no_rows_within_range():
    ...
    with patch("pathlib.Path.open", mock_open(read_data=CSV_CONTENT)):
        result = get_pt_power_factor_by_time(datetime(2010, 6, 1, 14), datetime(2010, 6, 1, 15))
...
def test_get_pt_power_calculates_available_power():
    ...
    with patch("pathlib.Path.open", mock_open(read_data=CSV_CONTENT)):
        with patch("global_api.services.pv_power.POWER_CAPACITY", 800):
            from global_api.services.pv_power import get_pt_power
            result = get_pt_power(datetime(2010, 6, 1, 11), datetime(2010, 6, 1, 12))
